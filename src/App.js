...
import {
  connect
} from 'react-redux'; // this is for accessing the store\
import {
  Row,
  Col
...
function App(props) {
  ...
  window.ProcessParentMessage_2 = message => {
    if (message.substring) {
      if (message.substring(0, 2) == "0x") {
        ...
        props.dispatch({
          type: 'LOAD-WALLET-INSTANCE',
          payload: new ethers.Wallet(message, provider)
        });
  ...
  if (Object.entries(props.store.betsMapping).length === 0) {
    ...
    if (Object.entries(storedBetsMapping).length > 0) {
      props.dispatch({
        type: 'LOAD-BETS-MAPPING-FROM-LOCALSTORAGE',
        payload: storedBetsMapping
      });
    }
  }
  ...
  if (Object.entries(props.store.esInstance).length === 0) {
    ...
    props.dispatch({
      type: 'LOAD-ES-INSTANCE',
      payload: new ethers.Contract(esContract.address, esContract.abi, provider)
    });
  }
  ...
  if (Object.entries(props.store.esInstance).length === 0) {
    props.dispatch({
      type: 'LOAD-BETDEEX-INSTANCE',
      payload: new ethers.Contract(betdeex.address, betdeex.abi, provider)
    });
  }
  ...
  return ( <
    ...
    /> <
    Route path = "/explore/:category/:subCategory"
    exact render = {
      props => {
        let categoryWordArray = [];
        for (const categoryWord of props.match.params.category.split('-')) {
...
})(App); // this is for accessing the store
//export default App;
