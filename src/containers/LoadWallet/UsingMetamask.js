...
class UsingMetamask extends Component {
  ...
  componentDidMount = async () => {
    ...
    if(window.ethereum) {
      ...
      const onCorrectNetwork = window.web3.currentProvider.networkVersion === (network === 'homestead' ? '1' : '4');
      ...
      if(!onCorrectNetwork) {
        ...
        const ethersWalletInstanceOfMetamask = metamaskWeb3Provider.getSigner();

        const userAddress = await ethersWalletInstanceOfMetamask.getAddress();
        ethersWalletInstanceOfMetamask['address'] = userAddress;

        this.props.dispatch({ type: 'LOAD-WALLET-INSTANCE', payload: ethersWalletInstanceOfMetamask });
